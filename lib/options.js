...
exports.readCmdOptions = function() {
	...
	var SWITCHES = [
		...
		['-s', '--statsd STATSD_FILE', "optional statsd configuration JSON file (defaults to ./conf/statsd.json)"],
	...
	//Handle the --statsd switch
	parser.on('statsd', function(name, value) {
	    options.statsd_file = value;
	});
	...
	// 3) statsd config
	var statsd_config_JSON = fs.readFileSync(options.statsd_file, "ascii");
	var statsd_config = JSON.parse(statsd_config_JSON);

	// all is appended to the "options" object.
	options.credentials = creds;
	options.metrics_config = metrics_config;
	options.statsd_config = statsd_config;
